...
namespace TinyBytes.Utils
{
    public static class SimplePool
    {
        ...
        public class Pool
        {
            ...
            public GameObject Spawn(Vector3 pos, Quaternion rot, Transform parent = null, bool enable = true)
            {
                GameObject obj;
                ...
                else
                {
                    ...
                    if (obj == null)
                    {
                        ...
                        return Spawn(pos, rot, parent, enable);
                    }
                }
            ...
            public void Despawn(GameObject obj)
            {
                ...
                if (!_inactive.Contains(obj))
                {
                    // We ensure to avoid adding same instance multiple times
                    _inactive.Push(obj);
                }
            }
        ...
        /// call that function.
        /// </summary>
        static public GameObject Spawn(GameObject prefab, Vector3 pos, Quaternion rot, Transform parent = null, bool enable = true)
        {
            Init(prefab);

            return _pools[prefab.GetInstanceID()].Spawn(pos, rot, parent, enable);
        }

        static public T Spawn<T>(T prefab, Transform parent, bool enable = true) where T : MonoBehaviour
        {
            T obj = Spawn(prefab.gameObject, parent.position, parent.rotation, parent, enable).GetComponent<T>();

            return obj;
        }

        static public T SpawnComponent<T>(T prefab, Transform parent, bool enable = true) where T : Component
        {
            T obj = Spawn(prefab.gameObject, parent.position, parent.rotation, parent, enable).GetComponent<T>();
